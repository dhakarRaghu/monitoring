...
export function Sidebar() {
  ...
  return (
    <aside className="w-[260px] h-screen shrink-0 overflow-hidden bg-background-secondary border-r border-card-border flex flex-col relative">
      ...
      <div className="relative p-6 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-card-elevated border border-card-border flex items-center justify-center">
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" className="text-foreground">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
          <div>
            <h1 className="text-[15px] font-semibold text-foreground tracking-tight">AI Mentor</h1>
            <p className="text-[11px] text-foreground-tertiary">Growth command center</p>
          </div>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-200 group ${
                isActive
                  ? "bg-card-elevated text-foreground border border-card-border"
...
function DashboardIcon({ active }: { active: boolean }) {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className={`transition-colors ${active ? "text-white" : "text-foreground-tertiary group-hover:text-foreground-secondary"}`}>
